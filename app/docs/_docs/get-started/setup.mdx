...
#### ⚠️ _API is not fully released, caution when using_
...
const conversation = await scout9.conversationCreate({
  customer: customerId,
  agent: agentId,
  environment: "phone", // This will attempt to contact-support via SMS
...
const conversationId = await scout9
  .conversationCreate({
    customer: customerId,
    agent: agentId,
    environment: "phone", // This will attempt to contact-support via SMS
...
for (const customerResponse of anticipatedCustomerResponses) {
  const generatedResponse = await scout9
    .generate({
...
const contextFields: ConversationContextField[] = [
  {
    id: "determineSize",
    context:
      "Determine what size pizza the customer wants, we have small, medium, and large",
    conditions: [
      {
        conditions: [
          {
...
const customEntities = [
  {
    ...
    option: "small",
    languages: ["en"],
    text: ["small", "personal", "individual", "6-inch", "8-inch"],
  },
...
await scout9.message({
  convo: conversationId,
  message: "Hey there, would you like a free pizza?",
});
```
